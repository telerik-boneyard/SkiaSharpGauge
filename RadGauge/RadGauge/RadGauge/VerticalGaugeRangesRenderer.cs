...
namespace RadGauge
{
    public class VerticalGaugeRangesRenderer : IGaugePartRenderer
    {
        ...
        public void Render(SKCanvas canvas, SKRect layoutSlot)
        {
            ...
            {
                var bottom = VerticalGaugeIndicatorRenderer.GetTickPosition(ranges[i], min, max, layoutSlot);
                var top = VerticalGaugeIndicatorRenderer.GetTickPosition(ranges[i + 1], min, max, layoutSlot);
